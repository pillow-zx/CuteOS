...
static int test_get_set_fd_flags(void)
{
	...
	failed += expect_ret("F_GETFD after set",
	...
	failed += expect_ret("F_GETFD after clear",
...
static int test_error_paths(void)
{
	...
	failed += expect_ret("F_GETFD invalid fd", fcntl(-1, F_GETFD, 0),
			     -EBADF);
...
static int test_reused_fd_clears_cloexec(void)
{
	...
	fd = openat(AT_FDCWD, "/fcntl_fd_reuse_a",
		    O_CREAT | O_RDWR | O_TRUNC, 0644);
	if (fd < 0) {
	...
	next = openat(AT_FDCWD, "/fcntl_fd_reuse_b",
		      O_CREAT | O_RDWR | O_TRUNC, 0644);
	if (next < 0) {
...
static int test_exec_closes_cloexec_fd(void)
{
	...
	fd = openat(AT_FDCWD, "/fcntl_fd_exec",
		    O_CREAT | O_RDWR | O_TRUNC, 0644);
	if (fd < 0) {
