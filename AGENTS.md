...
Prioritize asking the user to obtain all necessary information rather than
guessing the requirements

When uncertain about ABI, Linux compatibility, subsystem ownership, or design
boundaries, use `$grill-with-docs`, else use `$grill-me`
...
- Use compiler extensions provided in include/compiler at interface definitions 
  to enhance semantics, delegating more checks to the compiler.
- Use the utilities provided in include/kernel/tools.h, include/kernel/bitops.h, 
  and include/kernel/cleanup.h for management.
- For simple field-access-like functions, they may be placed directly in header 
  files as long as abstraction is not leaked.
...
- Full boot and kernel self-test path: `make qemu`(When user-mode tests need to be run, use interactive operations; do not use timeout x make qemu).
