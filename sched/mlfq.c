...
{
	task->sched.time_slice = mlfq_level_slice(task->sched.sched_level);
	task->sched.sched_ticks = 0;
}

static void boost_task(struct task_struct *task)
{
	task->sched.sched_level = 0;
	reset_budget(task);
}
...
{
	task->sched.sched_level = 0;
	task->sched.time_slice = mlfq_level_slice(0);
	task->sched.sched_ticks = 0;
	task->sched.enqueue_jiffies = 0;
}
...
void mlfq_enqueue(struct task_struct *task)
{
	...
	BUG_ON(task->sched.sched_level >= SCHED_MLFQ_LEVELS);
	BUG_ON(!list_empty(&task->sched.run_list));

	list_add_tail(&task->sched.run_list, &queues[task->sched.sched_level]);
	task->sched.enqueue_jiffies = jiffies;
	mark_nonempty(task->sched.sched_level);
	runnable_count++;
}

void mlfq_dequeue(struct task_struct *task)
{
	if (!task || list_empty(&task->sched.run_list))
		return;

	uint8_t level = task->sched.sched_level;
	BUG_ON(level >= SCHED_MLFQ_LEVELS);
