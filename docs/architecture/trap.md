...
- exec 构造新的用户返回 frame。
- signal 将 trap frame 转换为 Linux riscv64 `sigcontext`，写入用户栈上的
  `rt_sigframe`。
...
同步异常先经过 `trap_classify_exception()`。用户来源异常只能进入 syscall、
page fault 或 forced signal；内核来源的未处理同步异常统一诊断后 panic。内核
uaccess 引发的页故障仍可进入 MM，但 MM 无法修复时必须 panic，不能终止一个
无辜的用户任务来掩盖内核错误。
...
flowchart TD
    ...
    Classify["trap_classify_exception"]
    Fault["page fault<br/>do_page_fault"]
    Signal["forced signal<br/>siginfo_t"]
    ...
    IRQ -->|"no"| Classify
    Classify -->|"syscall"| Ecall --> Return
    Classify -->|"page fault"| Fault --> Return
    Classify -->|"user signal"| Signal --> Return
    Classify -->|"kernel fatal"| Panic
```
...
| 用户未对齐访问 | instruction/load/store misaligned | `SIGBUS/BUS_ADRALN` |
...
forced signal 与普通 signal 共用 signal 子系统。task pending 与 group shared
pending 都为每个标准 signal 保存一个 `siginfo_t` slot；pending bit 从 0 变 1
时保存首个 payload，同号再次发送只合并 bit，不覆盖首个信息。
