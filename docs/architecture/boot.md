...
`AT_PHNUM`、`AT_PAGESZ`、`AT_ENTRY`、身份条目、`AT_SECURE`、`AT_RANDOM` 与
`AT_EXECFN`。`AT_PHDR` 只在 program-header table 已由 `PT_LOAD` 映射给用户时
提供，否则 exec 失败。`AT_RANDOM` 来自当前 mtime/task-state 弱随机源，不是密码学熵。
静态 musl 可借此发现其 `PT_TLS` 描述并初始化初始线程 TLS。
