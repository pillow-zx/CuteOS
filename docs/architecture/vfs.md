...
VFS poll 通过通用 wait registrar 暴露 readiness source：
...
file operation 的 `poll()` 返回当前 readiness，并通过 `wait_register()` 登记
等待队列；registration 错误使用负 errno 传播。ppoll/pselect 在进入
`wait_complete()` 前 pin 稳定 file snapshot。epoll 使用 eventpoll 私有锁、
